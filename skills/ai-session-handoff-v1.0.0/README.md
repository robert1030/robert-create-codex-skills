...
版本 1.0.0。把當前 session 壓縮成一份下一個 AI session 可直接接手的交接文件。
...
**想改成只由手動呼叫**：在 `SKILL.md` 的 frontmatter 加一行 `disable-model-invocation: true`。這會讓 skill 不再佔用每回合的 context，但也不會自動觸發，需要自己輸入名稱。取捨說明見 `reports/02-design-decisions.md`（隨附於發行目錄，不在本包內）。
