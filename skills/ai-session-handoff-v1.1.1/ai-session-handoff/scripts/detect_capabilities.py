...
CAPABILITY_KEYS = (
    ...
    "plain_text_output",
    "validator_runnable",
    "reference_files_readable",
)

# 本 skill 的根目錄：pointer 內的相對路徑一律以此為基準。
...
def reference_files_readable(skill_root=None):
    """本 skill 的參考檔是否真的讀得到。

    宿主只載入 SKILL.md 時會是 False，此時照 SKILL.md 內的九節清單、
    第〇節閘門與狀態標記集合產出，並在交付訊息說明未載入逐節細則。
    """
    root = SKILL_ROOT if skill_root is None else skill_root
    probe = os.path.join(root, "references", "output-contract.md")
    try:
...
def detect(environ=None, which=None, temp_resolver=None):
    ...
    caps = {
        ...
        "reference_files_readable": reference_files_readable(),
    }
    caps["skill_root"] = SKILL_ROOT
...
def degradation_path(caps):
    ...
    if caps.get("reference_files_readable") is False:
        ladder.append("讀不到參考檔：依 SKILL.md 的九節清單與閘門產出，並在交付訊息說明未載入逐節細則")
