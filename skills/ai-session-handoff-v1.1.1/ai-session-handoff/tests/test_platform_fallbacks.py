...
print("== 參考檔可讀性 ==")
check("實際安裝下讀得到參考檔", caps_mod.reference_files_readable() is True)
check("skill_root 指向本套件根目錄",
      os.path.isfile(os.path.join(caps["skill_root"], "SKILL.md")))
check("參考檔缺席時回報 False",
      caps_mod.reference_files_readable(os.path.join(temp_dir, "not-a-skill-root")) is False)
no_ref_ladder = caps_mod.degradation_path({"temp_dir": False, "filesystem_write": False,
                                           "validator_runnable": True,
                                           "reference_files_readable": False})
check("降級階梯含讀不到參考檔的處置",
      any("讀不到參考檔" in step for step in no_ref_ladder))
check("讀不到參考檔不影響其他能力判定",
      caps_mod.choose_mode({"filesystem_write": True, "temp_dir": "/x",
                            "plain_text_output": True,
                            "reference_files_readable": False}) == "agent")
