# Popups（來源文件無 TOC 對應，依資料夾結構歸類） > popups/return_python.html > return
...
You can return a value for the procedure. The text specified for the specified in the Description cell (the value of the Command property) of the return step is appended to the response of the call step). Steps nested inside return steps are never executed.
