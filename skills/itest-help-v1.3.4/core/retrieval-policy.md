...
3. **指令表沒有列出的名稱，不是 iTest 直譯器指令。** 這類名稱若出現在 iTest 層，它屬於某個 Action 或某個欄位的語意，必須查該 Action 的說明頁，不得以直譯器指令的身分推論其寫法。
...
6. **範例形狀可能不在該 Action 自己的頁面上。** 部分 Action 說明頁的 Tcl 與 Python 範例在原始文件中是螢幕截圖，知識庫只保留圖片標記，沒有可引用的文字，`action_if.htm` 即為此例。這時必須到同一功能區塊的其他頁面取得佐證，例如 `action_then.htm`、`action_while.htm`、`action_switch.htm` 與 `loops_about.htm` 都有文字範例。不得因為該頁沒有文字範例就宣告知識庫沒有規定，也不得改用原生語言的形狀填補。
