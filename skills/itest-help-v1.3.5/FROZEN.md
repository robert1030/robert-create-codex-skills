...
本版是這個 skill 的**第一個減法版本**。前四版每一版都在加規則，本版不加任何規則，只處理累積下來的表述冗餘，並把「注意力預算」從口號變成可驗證的契約。

### 量測結果

改動前的實測數據：

| 版本 | `core/retrieval-policy.md` | agent 每次啟動必讀總量 |
| --- | ---: | ---: |
| v1.3.1 | 2,708 字元 | 13,555 字元 |
| v1.3.2 | 3,668 字元 | 14,798 字元 |
| v1.3.3 | 4,542 字元 | 16,128 字元 |
| v1.3.4 | 4,669 字元 | 16,316 字元 |

三版下來 policy 漲 72%，必讀總量漲 20%。另量測到 6 個核心概念在 4 個必讀檔案中重複、8 處規則含主觀詞（普遍偏低、很窄、真正指向、可能切題）而無法用測試守門。

**更正一項先前的說法**：v1.3.3 曾被描述為「兩節併一節的減法」，實際上它讓 policy 從 3,668 增加到 4,542，增幅 24%，屬於重構加擴充，不是減法。

### 本版做了什麼

1. **壓縮重複表述**：多輪檢索的 5 個判準併為一段、兩層執行模型的第 3 與第 6 條合併、查詢構詞去掉一個重複例子、`SKILL.md` 第 3 條與兩個 adapter 的對應段落改寫得更緊。合計省 452 字元。
2. **移除非必讀章節**：`SKILL.md` 的「佈署 profile」對照表與「資源索引」連結清單移除，合計 972 字元。這兩節是給人看的文件，不是 agent 回答問題的推理材料，且 `README.md` 已完整涵蓋，工作流各步驟也已內嵌所需連結。移除前已驗證沒有任何測試或驗證腳本依賴它們，且「能力邊界」中關於 profile 的行為約束原樣保留。
3. **注意力預算成為契約**：新增 `test_agent_startup_context_budget_is_capped`，把 agent 啟動必讀總量的上限訂為 15,500 字元。**日後想加規則，就必須先刪掉等量內容，或提出證據說明為何要調高預算**，否則這條紅燈。

改動後必讀總量為 14,942 字元，較 v1.3.4 減少 8.4%，回到 v1.3.2 水準。

### 本版沒有做什麼

- **沒有刪除任何規則**。41 條 `required_policy_text` 字串全數逐字保留，壓縮腳本在寫檔前自我驗證，任一條消失即中止。
- **沒有減少規則出現的檔案數**。v1.2.0 凍結的「紀律必須同時存在於四個檔案」原樣維持。
- **沒有動任何 `scripts/` 程式邏輯**，因此檢索行為不可能改變。

### 證據強度誠實聲明

- **有證據的部分**：冗餘確實存在（篇幅數據、重複度量測），這是硬數據。
- **沒有證據的部分**：**「減量會改善遵守率」完全沒有證據**。本版從未量測過遵守率與篇幅的關係。
- **一個反例**：Opus 版在 16,316 字元的條件下三題全對且主動取用完整內容，證明篇幅不是決定性因素。
- **一個因果警告**：三輪測試中被違反的是 v1.3.0 的片段規則，而該規則在 v1.3.1 至 v1.3.4 期間位置與篇幅都沒變。把違規歸因於篇幅是把相關當因果。
- **收益上限**：在保留全部守門字串的約束下，純粹壓縮表述只能省 2.5%，真正的減量來自移除非必讀章節。這說明**減法在現有凍結契約下空間有限**，若要更大幅度，必須刪規則或減少鏡射檔案數，兩者都會弱化守門或違反凍結契約。

因此本版的價值主要不在 8.4% 的減量本身，而在第 3 點的預算契約：它讓「規則只增不減」這個趨勢在機制上停下來。

### 第二批改動：檢索輸出的可見度與體積

本節記錄的是 v1.3.5 打包後、同日追加的第二批改動。**依使用者明確裁決，不另開 v1.3.6，就地併入 v1.3.5 重新打包**。這是房規「凍結契約，另開新版」的一次例外，成立條件是 v1.3.5 當日產生、僅安裝於本機、未對外交付，`SHA-256` 只記載於本地三份 manifest。第一批改動（表述壓縮與預算契約）的內容完全保留。

#### 起因

v1.3.5 第一批改動後的實機測試，題一再度失敗。查核發現：含 `Insert Step | Add a new step immediately following the selected step or procedure` 明文的 chunk **排名第 2、score 61、確實有回傳**，但 agent 只讀了工具輸出的前 2 KB 預覽，而該預覽**裝不下第 2 筆結果**。

實測體積組成：單次檢索輸出 53,320 位元組，其中 `locators` 佔 31,287（59%），`text` 僅佔 12,693（24%），第 1 筆結果本身就有 4,223 位元組。

**一項重要更正**：agent 自述「chunk-0001 完全沒有在任何一次查詢結果中見過」，經實測證偽，它排名第 2。**agent 的過程自述不可單獨採信，必須以外部實測交叉驗證**。

#### 兩項變更

- **`result_index`**：頂層新增一行一筆的結果索引，排在 `results` **之前**，內容為名次、`score`、`snippet` 或 `full`、`source_file` 與完整 `chunk_id`，8 筆約 1.3 KB。
- **`locators` 精簡**：檢索結果只回傳前 `LOCATORS_IN_SEARCH`（2）個代表項，新增 `locators_total` 標示完整數量。完整清單由 `inspect_chunk.py` 原樣提供，未受影響。

實測效果：

| 指標 | 改動前 | 改動後 |
| --- | ---: | ---: |
| 單次檢索輸出 | 53,320 位元組 | 24,079 位元組（減 55%） |
| 前 2 KB 可辨識的結果筆數 | 2/8 | **8/8** |

#### 附帶修補：介面文件漂移

`adapters/agent/retrieval-interface.md` 的回應範例停留在 v1.3.3 的欄位集，**v1.3.4 新增的 `truncated_count` 與 `next_action` 當時漏了同步**。本次一併補齊，並新增 `test_retrieval_interface_doc_matches_the_actual_response`，以實際回應的每一個頂層與結果欄位反向斷言文件，防止再次漂移。

#### 防迴歸

改動前先以未修改的 v1.3.5 建立 golden 基準，改動後比對**零差異**。排序、取窗、`chunk_id`、`score`、`text` 全部未變。新增 3 項契約斷言（29 增至 32），其中 `test_result_index_precedes_results_and_survives_preview_truncation` 直接斷言所有命中的 `chunk_id` 都能在前 2 KB 內被辨識，且 `result_index` 的鍵序必須排在 `results` 之前。

#### 能力邊界與證據強度

- **可量測且已驗證**：輸出體積減 55%、前 2 KB 可見筆數 2/8 增至 8/8。這是機制層事實，不依賴模型行為。
- **未驗證**：**看得到不等於會去看**。本次改動不保證 agent 會據 `result_index` 行動，這一點與 v1.3.4 的 `next_action` 同樣是未證實的假設。
- **適用限制**：`result_index` 的價值依賴「大輸出被截成預覽」這個執行環境特性。在不截斷的環境，它只是多出的 1.3 KB。其他平台（Chat Web 沙箱、Codex）的截斷行為未經驗證。
