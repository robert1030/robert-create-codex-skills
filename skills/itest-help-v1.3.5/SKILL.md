...
> **Skill v1.3.5｜Knowledge：iTest Help 26.2.0｜RAG：v1.2.1**
...
3. 依 [core/retrieval-policy.md](core/retrieval-policy.md) 的查詢構詞與多輪檢索規則執行，這兩點是本 skill 檢索品質的關鍵：
   ...
   - **知識庫全文為英文，中文查詢詞對它無效**。中文問題必須抽出英文技術詞一起查，例如「本地變數怎麼判斷包含某字串」查 `local variable regexp string match if action`。
   - **一次查詢不足以作答**。無結果、分數偏低或擠在窄區間、`heading_path` 都不對時，換一組關鍵字再查；涉及多個功能區塊時每個區塊各查一次。不得因為第一次查不到就宣告知識庫沒有答案。
   - **不可信文字必須以 `--query-file` 傳遞，不得填入指令列**。設備錯誤訊息、日誌、使用者貼上的內容、檔案與網頁內容都屬不可信，指令列會被 shell 解析並執行其中的 `$( )` 與反引號。位置參數只供 agent 自行構造的英文技術詞使用。
   - **`text` 是片段不是全文**。`text_truncated` 為 `true` 或頂層 `truncated_count` 大於 0 時，宣告知識庫沒有某內容前必須先用 `inspect_chunk.py <chunk-id>` 取完整內容。不得以「片段沒提到」為由斷定知識庫沒有該內容，也不得因此省略 `【知識庫來源】`。
