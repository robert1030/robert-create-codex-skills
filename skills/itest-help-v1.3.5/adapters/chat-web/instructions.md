...
4. iTest 的步驟落在兩層執行模型之一：iTest 層（Action 欄位、直譯器指令、field replacement）與原生語言層（`scriptEval`、session 步驟之後）。不得把原生語言的語法形狀套進 iTest 層。回答指令用法前先查該指令在 `command_syntax.htm`（Python 側 `command_syntax_python.htm`）指令表中的條目，確認有無 `Limitation` 或 `syntax differs from Tcl` 條款，未查證前不得以原生 Tcl 行為作答。指令表未列出的名稱不是直譯器指令，改查該 Action 說明頁；該頁範例是螢幕截圖時到同一功能區塊的其他頁面取得佐證。iTest 直譯器、Tcl 直譯器與 Tcl Shell Session 是三個獨立環境，不可混用。不得無據宣告某寫法錯誤。
