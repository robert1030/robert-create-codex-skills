...
# v1.3.5 起，agent 每次啟動的必讀總量設上限，防止規則無節制堆疊。
AGENT_CONTEXT_BUDGET = 15500
...
class ProfileTests(unittest.TestCase):
    ...
    def test_agent_startup_context_budget_is_capped(self) -> None:
        """v1.3.5：注意力預算是契約，不是口號。

        每次啟動必讀的檔案總量設上限。想加規則就得先刪掉同等份量的內容，
        否則這條紅燈。歷史上必讀量從 v1.3.1 的 13,555 字元漲到 v1.3.4 的 16,316，
        每一版都只加不減，這條斷言就是為了讓那個趨勢停下來。
        """
        must_read = (
            "SKILL.md",
            "core/query-policy.md",
            "core/retrieval-policy.md",
            "core/source-policy.md",
            "core/version-policy.md",
            "adapters/agent/instructions.md",
            "adapters/agent/claude-code.md",
        )
        total = sum(len((ROOT / name).read_text(encoding="utf-8")) for name in must_read)
        self.assertLessEqual(
            total, AGENT_CONTEXT_BUDGET,
            f"agent 必讀量 {total} 字元已超過預算 {AGENT_CONTEXT_BUDGET}，"
            "加規則前請先刪掉等量內容，或提出證據說明為何要調高預算",
        )

    def test_result_index_precedes_results_and_survives_preview_truncation(self) -> None:
        """v1.3.5：單筆結果就超過 2KB，第二名以後在預覽中不可見，result_index 補這個洞。

        斷言索引排在 results 之前，且所有命中的 chunk_id 都能在前 2KB 內被辨識。
        """
        completed = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "search_itest_help.py"),
             "Test Case Editor Insert menu Session step Tcl Shell", "--limit", "8"],
            capture_output=True, text=True, encoding="utf-8", check=True)
        raw = completed.stdout.encode("utf-8")
        payload = json.loads(completed.stdout)
        self.assertIn("result_index", payload)
        self.assertEqual(len(payload["result_index"]), len(payload["results"]))
        keys = list(payload)
        self.assertLess(keys.index("result_index"), keys.index("results"),
                        "result_index 必須排在 results 之前，否則被預覽截斷就失去意義")
        preview = raw[:2048].decode("utf-8", "ignore")
    ...
    def test_search_trims_locators_but_inspect_chunk_keeps_them(self) -> None:
        ...
        target = next(r for r in results if r["locators_total"] > search_itest_help.LOCATORS_IN_SEARCH)
        self.assertLessEqual(len(target["locators"]), search_itest_help.LOCATORS_IN_SEARCH)
        self.assertGreaterEqual(len(target["locators"]), 1, "引用需要至少一個可驗證的來源位置")

        full = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "inspect_chunk.py"), target["chunk_id"]],
            capture_output=True, text=True, encoding="utf-8", check=True)
        record = json.loads(full.stdout)["record"]
        self.assertEqual(len(record["locators"]), target["locators_total"],
                         "inspect_chunk 必須保留完整 locators")

    def test_retrieval_interface_doc_matches_the_actual_response(self) -> None:
        """介面文件漂移過一次（v1.3.4 的欄位沒同步），用斷言擋住第二次。"""
        completed = subprocess.run(
            [sys.executable, str(ROOT / "scripts" / "search_itest_help.py"), "regexp command", "--limit", "3"],
            capture_output=True, text=True, encoding="utf-8", check=True)
        payload = json.loads(completed.stdout)
        doc = (ROOT / "adapters" / "agent" / "retrieval-interface.md").read_text(encoding="utf-8")
        for key in payload:
            self.assertIn(f'"{key}"', doc, f"retrieval-interface.md 未記載頂層欄位 {key}")
        for key in payload["results"][0]:
            self.assertIn(f'"{key}"', doc, f"retrieval-interface.md 未記載結果欄位 {key}")
