...
def _media_block_from_occurrence(
    ...
    *,
    ...
    try:
        ...
        if inspection.visual_class in {"logo", "icon", "photo", "decorative"}:
            return context.block(
                "image", "", location=location, heading_path=list(heading_path),
                content_origin="derived_normalization", required=False, status="skipped", verbatim=False,
                metadata={**metadata, "skip_reason": "non_required_visual"},
            )
        result = ocr_image(image_bytes, languages=context.ocr_languages)
        if inspection.width <= 400 and inspection.height <= 400 and len(result.text.strip()) < 30:
            return context.block(
                "image", "", location=location, heading_path=list(heading_path),
                content_origin="derived_normalization", required=False, status="skipped", verbatim=False,
                metadata={**metadata, "skip_reason": "small_logo_or_icon_candidate", "ocr_candidate_rejected": result.text},
            )
...
def _shape_numbered_items(items: list[dict[str, Any]], excluded: set[int]) -> tuple[list[str], set[int]]:
    candidates = [
        ...
        and not re.fullmatch(r"(?:PAGE\s*)?\d+", item["text"], flags=re.IGNORECASE)
        and item["text"].replace(" ", "").upper() not in {"WEI", "LIN", "ENGLISH", "WEILINENGLISH"}
    ]
    result: list[str] = []
    used: set[int] = set()
...
def _drawingml_parse(path: Path, context: AdapterContext) -> DocumentIR:
    ...
    for page_number, items in enumerate(pages, start=1):
        for item in sorted(items, key=lambda value: (value["y"], value["x"])):
            text = item["text"]
            if not text or item["anchor_index"] in used_by_page[page_number]:
                continue
            if "TikTok" in text and ("?" in text or "？" in text):
                blocks.append(context.block(
                    "heading", text,
                    location=Location(page=page_number, bbox=[item["x"], item["y"], item["x"] + item["width"], item["y"] + item["height"]]),
                    heading_path=[],
    ...
    for page_number, items in enumerate(pages, start=1):
        rows, used_table = _shape_table(items)
        if rows:
            section_title = next((item for item in items if item["text"] == "影片單字"), None)
            blocks.append(context.block(
                "heading", "影片單字",
                location=Location(
                    page=page_number,
            ...
            header = ["編號", "單字", "IPA", "詞性與中文定義"] if len(rows[0]) == 4 else [f"欄位 {index}" for index in range(1, len(rows[0]) + 1)]
        ...
        for item in items:
            if not item["text"] or item["anchor_index"] in used_by_page[page_number]:
                continue
            compact = item["text"].replace(" ", "").upper()
            is_noise = compact in {"WEI", "LIN", "ENGLISH", "WEILINENGLISH", "本篇講解", "PAGE", "PAG", "PAGE1", "PAGE2"} or bool(re.fullmatch(r"(?:PAGE|PAG|PAGE)?\d+", compact))
            blocks.append(context.block(
                "paragraph", item["text"],
                location=Location(page=page_number, bbox=[item["x"], item["y"], item["x"] + item["width"], item["y"] + item["height"]]),
                heading_path=[],
                ...
                metadata={
                    "drawingml_anchor_index": item["anchor_index"],
                    **({"skip_reason": "page_or_brand_noise"} if is_noise else {}),
                },
            ))
