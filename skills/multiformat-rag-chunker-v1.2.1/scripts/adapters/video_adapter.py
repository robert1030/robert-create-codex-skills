    ...
    if has_video:
        ...
        for frame_index, timestamp in enumerate(sorted(set(frame_times)), start=1):
            ...
            if not _extract_frame(path, timestamp, output):
                continue
            result = ocr_image(output, languages=context.ocr_languages)
            ...
            block = context.block(
                ...
                metadata={"visual_class": "screen_capture", "ocr_confidence": result.confidence, "ocr_quality": result.quality},
            )
