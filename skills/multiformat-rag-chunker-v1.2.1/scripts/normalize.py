...
def remove_page_noise(text: str) -> tuple[str, list[str]]:
    ...
    for line in normalize_nfc(text).splitlines():
        ...
        if PAGE_FOOTER_RE.match(compact):
            ...
            continue
        if no_space in HEADER_NOISE:
            changes.append("repeated_brand_header_removed")
            continue
        lines.append(line)
...
def _remove_repeated_headers(blocks: list[Block]) -> None:
    candidates = [block.text.strip() for block in blocks if block.location.page and block.type in {"heading", "paragraph"} and len(block.text.strip()) <= 80]
    counts = Counter(candidates)
    repeated = {value for value, count in counts.items() if value and count >= 2 and value.upper() in HEADER_NOISE}
    for block in blocks:
