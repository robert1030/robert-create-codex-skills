...
PDF 原生文字不足的頁面以固定倍率渲染成頁面層 asset。非空白頁優先比對 `prepare_visual_review.py` 產生的 hash-bound 原生視覺回覆；有回覆時建立一個主要 `full_page_scan` Block，頁內非 decoder 圖片改為非必要 `covered_by_full_page_processing`，避免對同一頁重複 OCR。空白頁以 `blank_page` 跳過。沒有視覺回覆時才使用全頁 OCR；OCR 失敗必須保留 `native_visual_review_required`，不得因同頁 QR 成功而略過。
...
## 共用 capability routing 契約

所有 Adapter 在需要視覺補強時都必須呼叫 `AdapterContext.visual_route`，不得自行決定 LLM multimodal 不可用，也不得直接呼叫無 admission 的 OCR。原生結構 parser、alt text、QR／Barcode decoder 或可靠 machine payload 可先完成內容；只有仍需視覺語意的 asset 才建立 routing event。

每個 OCR 呼叫必須明示共用 `OCRAdmission`。`scripts/validate.py` 會交叉核對 asset SHA-256、selected lane、LLM attempt、OCR admission 與 Block evidence；Adapter 內部條件分支不能取代此獨立驗證。
