...
| `scripts/prepare_visual_review.py` | PDF 掃描頁原生視覺交接 | Agent 已具有原生視覺能力時，先產生頁面資產與 hash-bound 工作單，實際審核後再執行 Chunker。缺少 Tesseract 不能略過此路徑。 |
| `--capability-evidence` | Agent 與腳本的原生 LLM multimodal feature detection | 狀態缺失時維持 `unknown`。只有明確 fallback evidence 可放行共用 OCR admission。 |
...
掃描型 PDF 在 OCR 失敗但仍可由 Agent 原生視覺處理時，失敗原因必須先維持 `native_visual_review_required`。完成實際審核或明確證明所有原生及等效路徑不可用前，不得把單一 OCR backend 缺失提升為已耗盡能力。

`processing-report.json` 的 `source_metadata.capability_routing` 是每次選路的機器可讀證據。它不改變固定輸出數量，也不把任一特定工具列為使用者前置。
