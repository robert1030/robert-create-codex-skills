...
def _append_image(path: Path, tag, context: AdapterContext, blocks: list, heading_path: list[str], index: int) -> None:
    ...
    if inspection.qr_payloads:
        ...
        inspection.visual_class == "screen_capture"
        and visual_review is not None
        and visual_review.review_mode == "semantic_summary"
    ):
        route, _admission = context.visual_route(
            reference, asset_sha256, review_present=True,
        ...
        route, _admission = context.visual_route(
            reference, asset_sha256, review_present=False,
            visual_class=inspection.visual_class, native_structured_parser_status="insufficient",
        )
        blocks.append(context.block(
            "image",
            ...
            metadata={
                **metadata,
                "skip_reason": "no_verified_machine_payload",
                "capability_route": route,
            },
        ))
        ...
        result = ocr_image(data, admission=admission, languages=context.ocr_languages)
