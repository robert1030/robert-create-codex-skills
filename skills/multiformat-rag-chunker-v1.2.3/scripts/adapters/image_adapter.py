...
def parse(path: Path, context: AdapterContext) -> DocumentIR:
    ...
    visual_review = (
        context.visual_semantics.lookup(path.name, asset_sha256)
    ...
    if inspection.qr_payloads:
        ...
        route, admission = context.visual_route(
            path.name,
            asset_sha256,
            review_present=visual_review is not None,
            visual_class=inspection.visual_class,
            native_structured_parser_status="not_applicable",
        )
        if visual_review is not None and visual_review.review_mode == "semantic_summary":
            blocks.append(context.block(
                "image", visual_review.summary, location=Location(asset_id=asset_id), heading_path=[title],
                content_origin="llm_visual_summary", required=True, critical=True, verbatim=False,
                metadata={
                    **metadata,
        ...
        if visual_review is not None and visual_review.review_mode == "dense_text":
            for unit in visual_review.text_units:
                blocks.append(context.block(
                    ...
                    metadata={
                        **metadata,
                        "visual_text_evidence": visual_review.evidence(),
                        ...
                        "density_metrics": visual_review.density_metrics,
            ...
            metadata={
                **metadata,
                ...
                "ocr_semantic_status": (
                    ...
                    else "not_run" if result.status == "blocked"
