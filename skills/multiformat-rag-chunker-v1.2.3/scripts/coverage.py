...
def primary_content_evidence(blocks: list[Block]) -> dict[str, Any]:
    """Keep auxiliary machine payloads from masking unresolved document body content."""

    successful_primary = [
        block for block in blocks
        if _is_verified(block)
        and block.type != "heading"
        and block.content_origin != "qr_decoder"
        and block.metadata.get("content_role") not in {"machine_payload", "supplementary", "blank"}
    ]
    unresolved_primary = [
        block for block in blocks
        if block.required
        and block.status in {"failed", "low_quality"}
        and block.metadata.get("content_role") not in {"machine_payload", "supplementary", "blank"}
    ]
    machine_payloads = [
        block for block in blocks
        if _is_verified(block) and block.content_origin == "qr_decoder"
    ]
...
def dense_text_evidence(blocks: list[Block]) -> dict[str, Any]:
    """Report page-level admission of independently validated dense visual text."""

    dense = [block for block in blocks if block.metadata.get("dense_text_required") is True]
    references = sorted({str(block.metadata.get("reference") or "") for block in dense})
    verified: list[str] = []
    unresolved: list[str] = []
    for reference in references:
        page_blocks = [block for block in dense if str(block.metadata.get("reference") or "") == reference]
        accepted = bool(page_blocks) and all(
            block.status == "success"
            and block.content_origin == "llm_visual_text"
            and block.verbatim is False
            and block.text.strip()
            and block.metadata.get("visual_text_evidence", {}).get("independent_validation_status") == "passed"
            for block in page_blocks
        )
        (verified if accepted else unresolved).append(reference)
    ratio, status = _ratio(len(verified), len(references))
    return {
...
def evaluate_source_coverage(
    ...
    *,
    ...
    primary_evidence = primary_content_evidence(blocks)
    metrics["primary_content"] = primary_evidence
    dense_evidence = dense_text_evidence(blocks)
    metrics["dense_text"] = dense_evidence
    ...
    if not primary_evidence["has_effective_main_content"]:
        fatal_reasons.append("no_effective_main_content")
    if not normalized_ok:
        fatal_reasons.append("normalized_document_invalid")
    if dense_evidence["eligible_page_count"] and dense_evidence["coverage_ratio"] != 1.0:
        fatal_reasons.append("dense_text_independent_validation_incomplete")
