...
from visual_semantics import CapabilityEvidence, VisualSemantics, load_capability_evidence, load_visual_semantics
...
def process_source(
    ...
    context = AdapterContext(
        ...
        capability_evidence=capability_evidence or CapabilityEvidence(),
    )
    ...
    document.metadata["capability_routing"] = context.capability_routing_summary()
    ...
    parameters = {
        ...
        "partial_authorization": args.partial_authorization,
        "require_original_binary": args.require_original_binary,
        "forensic": args.forensic,
        "collection_id": item.collection_id,
        "collection_member_path": item.collection_member_path,
        "visual_semantics_review_count": visual_semantics.review_count if visual_semantics else 0,
        "visual_semantics_dense_review_count": visual_semantics.dense_review_count if visual_semantics else 0,
        "visual_text_validation_supplied": bool(args.visual_text_validation),
        "capability_evidence_supplied": bool(args.capability_evidence),
        "native_multimodal_capability_status": context.capability_evidence.status,
    }
    write_source_output(
        source_dir,
        document,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Required evidence label when the user explicitly authorizes partial chunks.",
    )
    ...
    parser.add_argument("--visual-text-validation", type=Path)
...
def main() -> int:
    ...
    if args.allow_partial_chunks and args.partial_authorization != "explicit_user_request":
        parser.error("partial_chunks_require_explicit_user_authorization")
    if args.partial_authorization and not args.allow_partial_chunks:
        parser.error("partial_authorization_without_partial_chunks")
    try:
        visual_semantics = load_visual_semantics(
            args.visual_semantics,
            args.input,
            args.visual_text_validation,
        )
        capability_evidence = load_capability_evidence(args.capability_evidence)
