...
from ocr import OCRAdmission, OCRResult, ocr_image
from output import failure_records
from visual import classify_visual, load_image
from visual_semantics import CapabilityEvidence, load_visual_semantics
...
class ImageSemanticContractTests(unittest.TestCase):
    ...
    def test_structured_screen_capture_requires_capability_evidence_without_ocr(self) -> None:
        with tempfile.TemporaryDirectory() as temp_name:
            temp = Path(temp_name)
            ...
            self.assertEqual(block["content_origin"], "placeholder")
            self.assertEqual(block["metadata"]["reason"], "native_multimodal_capability_evidence_required")
            self.assertEqual(block["metadata"]["capability_route"]["selected_lane"], "capability_evidence_required")
    ...
    def test_text_block_uses_accepted_ocr_result(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_name, mock.patch(
            ...
            document = parse_image(source, AdapterContext(
                provenance,
                Path(temp_name),
                "eng",
                capability_evidence=CapabilityEvidence(
                    status="unavailable",
                    evidence=("test_runtime_without_native_multimodal",),
                    failure_reason="native_multimodal_not_exposed",
                ),
            ))
        ...
        with mock.patch(
            ...
            result = ocr_image(
                ...
                admission=OCRAdmission(
                    admitted=True,
                    reason="native_multimodal_unavailable",
